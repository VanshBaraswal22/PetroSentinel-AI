...
export const ScenarioImpactView: React.FC<ScenarioImpactViewProps> = ({ initialSubTab = 'twin' }) => {
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Sub-navigation bar */}
      <div className="bg-white border border-slate-200 rounded-xl p-1.5 flex flex-col sm:flex-row items-stretch sm:items-center gap-2 max-w-2xl shadow-sm">
        <button
      ...
      {subTab === 'twin' && (
        <div className="space-y-6 animate-in fade-in duration-200">
          ...
          <div className="bg-white border border-slate-200 rounded-xl p-5 md:p-6 shadow-sm space-y-4">
            <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4">
              ...
              <div className="flex flex-wrap items-center gap-2">
                <span className="text-xs font-bold text-slate-500 uppercase tracking-wider w-full lg:w-auto block lg:inline">
                  Preset Scenarios:
                </span>

                <button
                  onClick={handlePresetHormuz}
                  className="px-3 py-2 rounded-lg text-xs font-bold bg-red-50 text-red-700 border border-red-200 hover:bg-red-100 flex items-center gap-1.5 transition-all min-h-[44px]"
                >
                ...
                <button
                  ...
                  className="px-3 py-2 rounded-lg text-xs font-bold bg-amber-50 text-amber-800 border border-amber-200 hover:bg-amber-100 flex items-center gap-1.5 transition-all min-h-[44px]"
                >
                ...
                <button
                  ...
                  className="px-3 py-2 rounded-lg text-xs font-bold bg-sky-50 text-sky-800 border border-sky-200 hover:bg-sky-100 flex items-center gap-1.5 transition-all min-h-[44px]"
                >
                ...
                <button
                  ...
                  className="px-3 py-2 rounded-lg text-xs font-bold bg-slate-100 text-slate-700 border border-slate-200 hover:bg-slate-200 flex items-center gap-1.5 transition-all min-h-[44px]"
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5 pt-2 border-t border-slate-100">
              ...
              <div className="bg-slate-50 border border-slate-200 rounded-xl p-4 space-y-3">
                <div className="flex items-center justify-between">
                  ...
                  <span className={`text-xs font-black font-mono px-2 py-0.5 rounded ${
                    hormuzCut > 50 ? 'bg-red-600 text-white' : hormuzCut > 0 ? 'bg-amber-500 text-white' : 'bg-slate-200 text-slate-700'
                  }`}>
                    {hormuzCut}% Cut
                  </span>
                </div>
              ...
              <div className="bg-slate-50 border border-slate-200 rounded-xl p-4 space-y-3">
                <div className="flex items-center justify-between">
                  ...
                  <span className={`text-xs font-black font-mono px-2 py-0.5 rounded ${
                    redSeaCut > 50 ? 'bg-red-600 text-white' : redSeaCut > 0 ? 'bg-sky-600 text-white' : 'bg-slate-200 text-slate-700'
                  }`}>
                    {redSeaCut}% Cut
                  </span>
                </div>
              ...
              <div className="bg-slate-50 border border-slate-200 rounded-xl p-4 space-y-3 sm:col-span-2 lg:col-span-1">
                <div className="flex items-center justify-between">
                  <span className="text-xs font-bold uppercase text-slate-800 flex items-center gap-1.5">
                  ...
                  <span className={`text-xs font-black font-mono px-2 py-0.5 rounded ${
                    opecCut > 2.0 ? 'bg-red-600 text-white' : opecCut > 0 ? 'bg-amber-600 text-white' : 'bg-slate-200 text-slate-700'
                  }`}>
                ...
                <input
                  ...
                  className="w-full accent-amber-700 h-2 bg-slate-200 rounded-lg cursor-pointer"
                />
                <div className="flex justify-between text-xs text-slate-500 font-medium">
                  <span>0.0 M bpd</span>
                  <span>2.5 M bpd</span>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            ...
            <div className="bg-white border border-slate-200 rounded-xl p-5 shadow-sm space-y-1">
              ...
              <div className="text-2xl sm:text-3xl font-extrabold text-slate-900 font-mono">
                ...
                <span className="text-xs text-slate-500 font-normal"> /bbl</span>
              </div>
              <div className="flex items-center gap-2 pt-1 text-xs">
                <span className={`font-bold font-mono ${metrics.priceSurgeDollar > 0 ? 'text-red-600' : 'text-emerald-600'}`}>
              ...
              <p className="text-xs text-slate-500 border-t border-slate-100 pt-2 mt-2">
                Est. pump price impact: <strong className="text-slate-800 font-mono">+₹{metrics.pumpImpactINR.toFixed(1)}/L</strong>
              </p>
            </div>
            ...
            <div className="bg-white border border-slate-200 rounded-xl p-5 shadow-sm space-y-1">
              ...
              <div className="text-2xl sm:text-3xl font-extrabold text-slate-900 font-mono">
                ...
                <span className="text-xs text-slate-500 font-normal"> M bpd</span>
              </div>
            ...
            <div className="bg-white border border-slate-200 rounded-xl p-5 shadow-sm space-y-1">
              ...
              <div className={`text-2xl sm:text-3xl font-extrabold font-mono ${metrics.netDeficitMBPD > 1.0 ? 'text-red-600' : 'text-slate-900'}`}>
            ...
            <div className={`border rounded-xl p-5 shadow-sm space-y-1 transition-all ${
              ...
              <div className={`text-2xl sm:text-3xl font-extrabold font-mono ${
                metrics.sprDaysToDepletion <= 5 ? 'text-red-700' : metrics.sprDaysToDepletion <= 10 ? 'text-amber-800' : 'text-slate-900'
              }`}>
              ...
              <p className="text-xs text-slate-500 border-t border-slate-200/60 pt-2 mt-2">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
            ...
            <div className="lg:col-span-7 bg-white border border-slate-200 rounded-xl p-5 md:p-6 shadow-sm space-y-4 flex flex-col justify-between">
              <div>
              ...
              <div className="w-full h-64 pt-2">
                <ResponsiveContainer width="100%" height="100%">
                  <AreaChart data={metrics.chartData} margin={{ top: 10, right: 10, left: -20, bottom: 0 }}>
                    ...
                    />
                    <ReferenceLine y={12.3} stroke="#dc2626" strokeDasharray="4 4" label={{ value: '3-Day Emergency Limit', fill: '#dc2626', fontSize: 10, position: 'insideTopRight' }} />
                    <Area type="monotone" dataKey="barrels" stroke="#1e3a8a" strokeWidth={2.5} fillOpacity={1} fill="url(#sprGradient)" />
                  </AreaChart>
                </ResponsiveContainer>
              </div>
            ...
            <div className="lg:col-span-5 bg-white border border-slate-200 rounded-xl p-5 md:p-6 shadow-sm space-y-4">
              <div className="flex items-center justify-between">
                <h3 className="text-sm font-bold text-slate-900 uppercase tracking-wider flex items-center gap-2">
                  <Factory className="w-4 h-4 text-blue-900" />
              ...
              <div className="space-y-3.5">
                {metrics.refineries.map((ref) => (
                  <div key={ref.name} className="p-3 rounded-lg border border-slate-200 bg-slate-50 space-y-1.5">
                    <div className="flex items-center justify-between">
                      <div>
                        <span className="text-xs font-bold text-slate-900">{ref.name}</span>
                        <span className="text-xs text-slate-500 ml-2">({ref.capacityMBPD} M bpd)</span>
                      </div>
                      <span className={`text-xs font-black px-2 py-0.5 rounded ${
                        ref.exposureScore >= 70 ? 'bg-red-600 text-white' : ref.exposureScore >= 40 ? 'bg-amber-500 text-white' : 'bg-emerald-600 text-white'
                      }`}>
                    ...
                    <div className="w-full h-2 bg-slate-200 rounded-full overflow-hidden">
                      <div 
                        className={`h-full transition-all duration-300 ${
                          ref.exposureScore >= 70 ? 'bg-red-600' : ref.exposureScore >= 40 ? 'bg-amber-500' : 'bg-emerald-600'
                        }`}
          ...
          <div className="bg-slate-900 text-slate-100 rounded-xl p-5 md:p-6 shadow-md space-y-3">
            ...
            <ul className="grid grid-cols-1 md:grid-cols-3 gap-3 text-xs text-slate-300">
              <li className="p-3 bg-slate-800/80 rounded-lg border border-slate-700">
                <strong className="text-white block mb-1">1. Padur & Mangalore Drawdown</strong>
              ...
              <li className="p-3 bg-slate-800/80 rounded-lg border border-slate-700">
                <strong className="text-white block mb-1">2. Spot Tanker Rerouting</strong>
              ...
              <li className="p-3 bg-slate-800/80 rounded-lg border border-slate-700">
                <strong className="text-white block mb-1">3. Hydrocracker Optimization</strong>
